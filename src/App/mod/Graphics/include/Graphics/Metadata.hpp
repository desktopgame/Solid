...
namespace Lib::Graphics::Metadata {
    ...
    const std::vector<Program> k_programs = std::vector<Program> {
        ...
        Program {
            ...
            "    PSOutput output;\n"
            "    output.outPosition = float4(input.worldPos, 1);\n"
        ...
        Program {
            ...
            "PSOutput psMain(Output input) : SV_TARGET {\n"
            "    PSOutput output;\n"
            "    output.outPosition = input.worldPos;\n"
            "    output.outColor = input.color;\n"
            "    return output;\n"
            "}\n"
            ,
        ...
        Program {
            ...
            "    float distance = length(input.worldPos - input.cameraPosition);\n"
            ...
            "\n"
            "    // 走査線の範囲を計算\n"
            "    float scanEffect = smoothstep(lowerY, lowerY + 0.1, input.worldPos.y) *\n"
            "                        smoothstep(upperY, upperY - 0.1, input.worldPos.y);\n"
            "    // float scanEffect = smoothstep(lineWidth, 0.0, abs(input.worldPos.y - scanLineY));\n"
